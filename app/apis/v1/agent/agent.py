...
async def create_agent(agent: AgentCreate, db_session: Session = Depends(get_db)):
    """API para la creación de agente"""
...
@router.get("/agent/{agent_id}", response_model=IResponseBase[AgentResponse])
...
@router.put("/agent/{agent_id}", response_model=IResponseBase[AgentResponse])
async def edit_agent(agent_id: int, new_agent: AgentCreate, db_session: Session = Depends(get_db)):
    ...
    updated_agent = crud.update_agent(db_session, currentAgent=agent_db, newAgent=new_agent)
    return IResponseBase[AgentResponse](response=updated_agent)


# Elimina Agente
@router.delete("/agent/{agent_id}", response_model=IResponseBase[str])
